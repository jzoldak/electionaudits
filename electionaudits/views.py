...
def kmresult(request, contest):
    """Generate Kaplan-Markov audit selections and results for selected ContestBatches"""

    contest = get_object_or_404(Contest, id=contest)

    result = contest.km_result()

    return render_to_response('electionaudits/kmresult.html',
                              {'contest': contest,
                               'result': result } )
...
def results(request):
    ...
    for cb in au_targeted:
        cb.min_margin = cb.contest.overall_margin  or  cb.contest.min_margin
